...
if (!('getTau' %in% ls()))
source('SimpleGillespie.R')
if (!is.loaded("MyYodzisInnesState")) dyn.load("myGruyere_model.so")
if (!is.loaded("MyYodzisInnesState")) stop("Didn't load...")
...
YodzisInnesPropNoImmi <- cmpfun(function(B, params)
{
    rates <- MyYodzisInnesFlux(B, params) 
    births <- (rates$growth > 0) * rates$growth + # Producer Births
              colSums(rates$assimilation) # Consumer Births
    ...
    # Transformation of propensities
    out <- c(births, deaths) * params$t.prime
    return(out)
})
...
YodzisInnesNu <- function(M, immigration = TRUE) {
    numspecies <- length(M)
    M_birth <- M_immigration <- diag(numspecies) * M
    # Creates Matrix with M on diagonal
    M_death <- M_birth * -1
