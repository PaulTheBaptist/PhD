if(!('MyYodzisInnesFlux' %in% ls())) source('MyModel.R')

pars <- do.call(ModelParamsSpec, preparams[-(11:12)], envir = .GlobalEnv)
...
if (immigration) {
    ...
    a_func <- function(args) {
        with(args, YodzisInnesPropNoImmi(B, params))
    }
}
nu <- YodzisInnesNu(M, immigration)
